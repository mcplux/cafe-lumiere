...
<template>
  ...
  <div class="mt-10">
    ...
    <div class="flex flex-col gap-3 mt-5">
      <div
        v-for="item in ordersStore.orderItems"
        :key="item.menuItem.id"
        class="relative border border-gray-200 p-2 flex flex-col sm:flex-row gap-3"
      >
        <button
        ...
        <div>
          <p class="font-bold text-lg">{{ item.menuItem.name }}</p>
          <p>
            Price:
            <span class="font-bold"
              >{{ formatCurrency(item.menuItem.price) }} x {{ item.quantity }}</span
            >
          </p>
          ...
          <p>
            Total:
            <span class="font-bold">{{ formatCurrency(item.menuItem.price * item.quantity) }}</span>
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mt-5">
      ...
      >
        <button
          ...
          @click="() => ordersStore.addOrderItem(item)"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
